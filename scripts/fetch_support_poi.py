...
from pipeline_schema import DEFAULT_CITY_ADCODES, DEFAULT_SUPPORT_KEYWORDS


OUTPUT_FIELDS = schema_fieldnames("poi_support.csv")
...
def find_source_row(source_id: str) -> dict[str, str]:
    for row in load_manifest():
        if row.get("source_id") == source_id:
            return row
    return {"source_id": source_id, "city": "ALL", "url_or_page_name": "", "target_table": "poi_support"}
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Fetch support POI from AMap.")
    ...
    if not api_key:
        append_blocker(
            ...
            "missing_api_key",
            "设置环境变量 AMAP_WEB_API_KEY 后再运行 fetch_support_poi.py。",
        )
        write_csv_rows(CLEAN_DIR / "poi_support.csv", OUTPUT_FIELDS, [])
        print("poi_support complete: rows=0 blocked=missing_api_key")
        return

    output_rows = []
    scopes = build_scopes()
    for scope in scopes:
        for keyword in DEFAULT_SUPPORT_KEYWORDS:
            query_batch_id = make_hash_id("poiquery", "support", scope["city"], scope["district"], keyword)
            for page in range(1, args.max_pages + 1):
                ...
                raw_path = RAW_API_DIR / "amap_poi_support" / f"{query_batch_id}_p{page}.json"
                ...
                for poi in pois:
                    ...
                    output_rows.append(
                        {
                            "poi_row_id": make_hash_id("poi", "support", duplicate_key, query_batch_id),
                            "poi_group": "support",
                            "city": scope["city"],
    ...
    for row in output_rows:
        ...
        if dedupe_key in seen:
            ...
            seen.add(dedupe_key)
    write_csv_rows(CLEAN_DIR / "poi_support.csv", OUTPUT_FIELDS, output_rows)
    print(f"poi_support complete: rows={len(output_rows)}")
